...
const { run } = require('../../src/commands/rm.js');

function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'todo-rm-test-'));
}
...
test('valid index removes the item, saves, prints confirmation, exit code 0', async () => {
...
test('nextId in the saved store is unchanged (never decremented) after removal', async () => {
  const dir = makeTmpDir();
  writeStore(dir, {
    nextId: 3,
    items: [
...
test('n = list.length is accepted (removes last item)', async () => {
  const dir = makeTmpDir();
  writeStore(dir, {
    nextId: 3,
    items: [
...
test('n = "-1" is rejected as INVALID_INDEX, no store write', async () => {
  const dir = makeTmpDir();
  writeStore(dir, { nextId: 2, items: [{ id: 1, text: 'a', done: false }] });
  const before = readStore(dir);
  const { stdout, stderr, err } = captureStreams();
