...
// the ⛔ DENIED block in the demo is NOT written by hand. This script builds a real
...
// green and re-runs the same hook to prove the allow path. If the hook's behaviour or
// wording ever changes, re-running `npm run demo` changes the asset — and if the hook
// stopped denying, this script would fail loudly rather than ship a flattering picture.
//
// Everything outside the DENIED block is scripted narration of the pipeline steps and is
...
const tasksDir = join(box, ".shapeup-sdlc", SLUG, "tasks");
...
function runHook() {
  const payload = JSON.stringify({
    ...
    tool_input: {
      ...
      skill_args: `--spec docs/shapeup-sdlc/${SLUG}/spec --feature ${SLUG} --single-pass`,
    },
    cwd: box,
  });
...
// (a) partial board → must DENY
writeBoard((t) => t.partial);
const denyRaw = runHook();
if (!denyRaw) {
  rmSync(box, { recursive: true, force: true });
  throw new Error("gate-l2 did not deny a partial board — refusing to render a demo that lies.");
}
const deny = JSON.parse(denyRaw).hookSpecificOutput;
if (deny?.permissionDecision !== "deny") {
  rmSync(box, { recursive: true, force: true });
  throw new Error(`expected permissionDecision "deny", got ${deny?.permissionDecision}`);
}

// (b) green board → must ALLOW (defer = empty stdout)
writeBoard(() => "done");
const allowRaw = runHook();
rmSync(box, { recursive: true, force: true });
if (allowRaw) throw new Error(`gate-l2 denied a green board: ${allowRaw}`);
...
const script = [
  ...
  ["", C.fg, 0],
  ["  ⛔ PreToolUse hook — DENIED   Skill(spec-evaluator)", C.red, 1],
  ...wrap(REASON).map((l) => ["     " + l, C.fg, 0]),
  ["", C.fg, 2],
  ["  ↑ verbatim stdout from hooks/gate-l2.mjs.", C.mag, 0],
  ["    Not a prompt. Not a reminder. The tool call", C.mag, 0],
  ["    never reached the evaluator.", C.mag, 3],
  ["", C.fg, 0],
...
writeFileSync(
  ...
  [
    ...
    "Generated by scripts/demo/record-demo.mjs (npm run demo).",
    "",
    "The DENIED block below is verbatim stdout captured from a real run of",
    ...
    denyRaw,
    "",
    "Raw hook stdout (green board): <empty> — exit 0, i.e. defer, EVAL allowed.",
    "",
  ].join("\n"),
);

console.log(`✅ verified: gate-l2 denied a partial board and allowed a green one`);
