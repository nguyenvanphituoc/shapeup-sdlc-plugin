...
function withTempCwd(fn) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'todo-cli-done-'));
...
test('marks a valid item done, saves store, exit 0', () => {
  withTempCwd(() => {
    writeStore({
      nextId: 3,
      items: [
    ...
    const { code, logs } = captureConsole(() => done(['1']));
...
test('n = 0 is rejected as INVALID_INDEX, no store write', () => {
  withTempCwd(() => {
    ...
    const { code, errors } = captureConsole(() => done(['0']));
...
test('n = list.length is accepted (last item)', () => {
  withTempCwd(() => {
    writeStore({
      nextId: 3,
      items: [
    ...
    const { code } = captureConsole(() => done(['2']));
...
test('n = list.length + 1 is rejected as INDEX_OUT_OF_RANGE, no store write', () => {
  withTempCwd(() => {
    ...
    const { code, errors } = captureConsole(() => done(['2']));
...
test('any positive n on an empty list is rejected as INDEX_OUT_OF_RANGE', () => {
  withTempCwd(() => {
    ...
    const { code, errors } = captureConsole(() => done(['1']));
...
test('n = "abc" is rejected as INVALID_INDEX, no store write', () => {
  withTempCwd(() => {
    ...
    const { code, errors } = captureConsole(() => done(['abc']));
...
test('n = "1.5" is rejected as INVALID_INDEX, no store write', () => {
  withTempCwd(() => {
    writeStore({ nextId: 2, items: [{ id: 1, text: 'Buy milk', done: false }] });

    const { code, errors } = captureConsole(() => done(['1.5']));
...
test('missing <n> is rejected as MISSING_INDEX, no store write', () => {
  withTempCwd(() => {
    ...
    const { code, errors } = captureConsole(() => done([]));
