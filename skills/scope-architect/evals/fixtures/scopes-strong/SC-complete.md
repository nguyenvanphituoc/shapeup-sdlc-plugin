...
`done`, `rm` and `archive` share the selector resolution and the same "resolve first, write once"
rule, so they are one chain. Splitting `archive` off would put the same invariant in two scopes.
