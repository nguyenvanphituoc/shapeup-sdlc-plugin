...
description: >
  ...
  from spec folder", "pick up the next task", or a WorkOrder dispatch (--order) from the
  tech-lead orchestrator. Implements a work order's acceptance criteria exactly — minimum
  code, surgical diffs, verified observable outcomes. Input: a WorkOrder envelope. Output:
  code + a WorkResult envelope. A pure worker: it never writes run-state, task boards,
  indexes, or ledgers — everything it used to write into shared files it returns as data
  for the orchestrator's ingest script.
---
