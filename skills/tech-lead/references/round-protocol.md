...
This loop is a **ratchet**: every attempt is scored against the last kept one, and the working
tree moves forward only when the score strictly improves. `t0-verify.mjs` makes that decision
and acts on the tree itself, inside the runtime — the orchestrator reads the decision, it never
makes it.
