...
- executor: task-executor via ingest-result
- status: failed
- `test/integration/cli.test.js` spawns `node bin/todo.js <args>` with `cwd` set to a: skipped (Not attempted — depends on TASK-008's dispatcher, which is itself blocked (see deviations).)
- `node --test test/integration/cli.test.js` passes: skipped (Not attempted — blocked.)
- Full round-trip: `add` → `list` shows the item pending → `done 1` → `list` shows it done →: skipped (Not attempted — none of add/list/done/rm command modules exist yet.)
- Missing store file: `list` on a fresh temp dir exits 0 with the empty message, does not: skipped (Not attempted — `list` command module does not exist yet.)
- Corrupted store file: write `not json {{{` to `./.todo.json` in the temp dir, then run any: skipped (Not attempted — blocked.)
...
- `test/integration/cli.test.js` spawns `node bin/todo.js <args>` with `cwd` set to a: skipped (Not attempted — depends on TASK-008's dispatcher, which is itself blocked (see deviations).)
- `node --test test/integration/cli.test.js` passes: skipped (Not attempted — blocked.)
- Full round-trip: `add` → `list` shows the item pending → `done 1` → `list` shows it done →: skipped (Not attempted — the round-trip requires add/list/rm, none of which exist yet.)
- Missing store file: `list` on a fresh temp dir exits 0 with the empty message, does not: skipped (Not attempted — `list` command module does not exist yet.)
- Corrupted store file: write `not json {{{` to `./.todo.json` in the temp dir, then run any: skipped (Not attempted — blocked.)
