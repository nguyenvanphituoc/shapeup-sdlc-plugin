...
- status: done
- `node bin/todo.js done 1` on a store with ≥1 item sets item 1's `done` to `true`, saves,: pass (node --test test/commands/done.test.js → 'done 1 on a store with >=1 item marks it done, saves, prints Done, exits 0' passed)
- Running `done 1` a second time (already done) is idempotent: same success message, exit 0,: pass (node --test test/commands/done.test.js → 'running done 1 a second time is idempotent: same message, exit 0, no error' passed)
- `node bin/todo.js done` (no index) exits 1 with an `E_MISSING_INDEX` message to stderr,: pass (node --test test/commands/done.test.js → 'done with no index exits 1 with E_MISSING_INDEX to stderr, store unchanged' passed)
- `node bin/todo.js done abc` exits 1 with an `E_INVALID_INDEX` message to stderr, store: pass (node --test test/commands/done.test.js → 'done abc exits 1 with E_INVALID_INDEX to stderr, store unchanged' passed)
- `node bin/todo.js done 2.5` and `node bin/todo.js done 3abc` are both rejected as: pass (node --test test/commands/done.test.js → 'done 2.5 and done 3abc are both rejected as E_INVALID_INDEX' passed)
- `node bin/todo.js done ""` (empty-string arg, if reachable via the shell) is rejected as: pass (node --test test/commands/done.test.js → 'done "" (empty-string arg) is rejected as E_INVALID_INDEX, never treated as index 0' passed)
- At min value (`1`, with ≥1 item): accepted, marks the first item done: pass (node --test test/commands/done.test.js → 'min value (1, with >=1 item): accepted, marks the first item done' passed)
- At max value (`items.length`, exact upper bound): accepted, marks the last item done: pass (node --test test/commands/done.test.js → 'max value (items.length): accepted, marks the last item done' passed)
- Below min (`0` or negative): rejected `E_INDEX_OUT_OF_RANGE`: pass (node --test test/commands/done.test.js → 'below min (0 or negative) rejected E_INDEX_OUT_OF_RANGE' passed)
- Above max (`items.length + 1`): rejected `E_INDEX_OUT_OF_RANGE`: pass (node --test test/commands/done.test.js → 'above max (items.length + 1) rejected E_INDEX_OUT_OF_RANGE' passed)
...
- executor: task-executor via ingest-result
- status: done
- `node bin/todo.js done 1` on a store with ≥1 item sets item 1's `done` to `true`, saves,: pass (node --test test/commands/done.test.js → 'done 1 on a store with >=1 item marks it done, saves, prints Done, exits 0' passes (12/12 total))
- Running `done 1` a second time (already done) is idempotent: same success message, exit 0,: pass (test 'running done 1 a second time is idempotent: same message, exit 0, no error' passes)
- `node bin/todo.js done` (no index) exits 1 with an `E_MISSING_INDEX` message to stderr,: pass (test 'done with no index exits 1 with E_MISSING_INDEX to stderr, store unchanged' passes)
- `node bin/todo.js done abc` exits 1 with an `E_INVALID_INDEX` message to stderr, store: pass (test 'done abc exits 1 with E_INVALID_INDEX to stderr, store unchanged' passes)
- `node bin/todo.js done 2.5` and `node bin/todo.js done 3abc` are both rejected as: pass (test 'done 2.5 and done 3abc are both rejected as E_INVALID_INDEX' passes)
- `node bin/todo.js done ""` (empty-string arg, if reachable via the shell) is rejected as: pass (test 'done "" (empty-string arg) is rejected as E_INVALID_INDEX, never treated as index 0' passes)
- At min value (`1`, with ≥1 item): accepted, marks the first item done: pass (test 'min value (1, with >=1 item): accepted, marks the first item done' passes)
- At max value (`items.length`, exact upper bound): accepted, marks the last item done: pass (test 'max value (items.length): accepted, marks the last item done' passes)
- Below min (`0` or negative): rejected `E_INDEX_OUT_OF_RANGE`: pass (test 'below min (0 or negative) rejected E_INDEX_OUT_OF_RANGE' passes)
- Above max (`items.length + 1`): rejected `E_INDEX_OUT_OF_RANGE`: pass (test 'above max (items.length + 1) rejected E_INDEX_OUT_OF_RANGE' passes)
- Empty store (`done 1` with 0 items): rejected `E_INDEX_OUT_OF_RANGE` (range `[1, 0]` is: pass (test 'empty store (done 1 with 0 items) rejected E_INDEX_OUT_OF_RANGE' passes)


## Execution Log — 2026-08-16 (todo-cli/complete-todo-r2-a1)
- executor: task-executor via ingest-result
- status: done
- `node bin/todo.js done` (no index) exits 1 with an `E_MISSING_INDEX` message to stderr,: pass (BUG-03 fix: lib/parse-index.js MissingIndexError message now `missing index`; bin/todo.js cmdDone renders `Error: ${err.message}`. Manual repro: `HOME=/tmp/bugtest node bin/todo.js done` -> stderr `Error: missing index`. node --test test/commands/done.test.js: 12/12 pass.)
- `node bin/todo.js done abc` exits 1 with an `E_INVALID_INDEX` message to stderr, store: pass (BUG-03 fix: InvalidIndexError message now `"<raw>" is not a valid index`. Manual repro: `done zz` -> stderr `Error: "zz" is not a valid index`. done.test.js line 76 updated to assert catalog text, passes.)
- Below min (`0` or negative): rejected `E_INDEX_OUT_OF_RANGE`: pass (BUG-03 fix: IndexOutOfRangeError message now `no todo at index <n>`. Manual repro: `done 7` on 1-item store -> stderr `Error: no todo at index 7`. done.test.js lines 135/137/148/159 updated to assert catalog text, all pass.)
