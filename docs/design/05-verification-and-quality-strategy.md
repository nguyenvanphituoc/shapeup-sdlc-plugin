...
| 2 | **Activation** | FPR, TPR over cross-skill hard negatives | **FPR 0.0** (n=67, 2026-08-12, Haiku 4.5); TPR **0.269** (18/67), diagnostic only | *TPR measures description quality.* It also measures the model's activate-versus-ask disposition: a large share of positives are deictic ("coach **this feedback**"), and a model that names the right skill and asks for the missing input scores as a miss. Quote FPR; treat TPR as a lead — and the 2026-08-12 re-measurement says so with evidence: across the two runs **eleven of twelve descriptions were byte-identical**, on the same model and the same harness, yet `task-executor` moved 0 → 0.833 and `shapeup` 0.429 → 0. Whatever that swing measures, it is not description quality. Movement in both directions is also the reason to trust the run: rate-limited probes scored as misses would depress every skill at once. | structural §16, `tools/trigger-eval.mjs` |
