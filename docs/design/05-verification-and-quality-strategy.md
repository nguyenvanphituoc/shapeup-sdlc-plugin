...
| 2 | **Build round** (product) | T0 green, round build gate, acceptance | T0 verdicts and round build gates are produced per run; **no acceptance-vs-baseline comparison is maintained** | *Acceptance in one context window generalizes.* It says nothing about what happens across one. | `kernel/verify/t0.mjs`, `spec-evaluator` |
...
| Rows | Home | Why there |
|---|---|---|
| 2 | [04 — Functional Design](04-functional-design.md) | The build round and its T0 verification are functional behaviour. |
